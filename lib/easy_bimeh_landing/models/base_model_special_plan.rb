...
module EasyBimehLanding
  # BaseModelSpecialPlan Model.
  class BaseModelSpecialPlan < BaseModel
    ...
    def self.from_hash(hash)
      ...
      unless hash['message'].nil?
        ...
        hash['message'].each do |structure|
          message << (SpecialPlan.from_hash(structure) if structure)
        end
      end
      extra_data = ExtraDataBaseModelSpecialPlan.from_hash(hash['extraData']) if
        hash['extraData']
      exception = hash['exception']

      # Create object from extracted values.
      BaseModelSpecialPlan.new(is_success,
                               status,
                               message,
                               extra_data,
                               exception)
